/**
 * \file QyncMainWindow.cpp
 * \author Darren Hatherley
 * \date 13th December, 2013
 * \version 0.9.5
 *
 * \brief Implementation of the QyncMainWindow class.
 *
 * \dep
 * - QyncMainWindow.h
 * - QyncManager.h
...
QyncMainWindow::QyncMainWindow( QyncManager * manager )
:	m_manager(0),
	m_tempPreset(0),
	m_prefsWindow(0) {
	...
	m_tempPreset = new QyncPreset();
	createWidgets();
	setManager(manager);

	if(m_manager) {
		QyncGuiPreferences * newPrefs = new QyncGuiPreferences(QyncManager::configurationDirectoryPath() + "guipreferences");
		m_manager->setPreferences(newPrefs);
	}
}


QyncMainWindow::~QyncMainWindow( void ) {
	if(m_tempPreset) {
		delete m_tempPreset;
		m_tempPreset = 0;
	}
...
void QyncMainWindow::createWidgets( void ) {
...
void QyncMainWindow::showPresetFromMenu( void ) {
	if(!m_manager) {
		qDebug() << "QyncMainWindow has no QyncManager";
		return;
	}
...
void QyncMainWindow::showPreset( int index ) {
	if(!m_manager) {
		qDebug() << "QyncMainWindow has no QyncManager";
		return;
	}
	...
	if(index >= 0 && index < m_manager->presetCount()) {
		QyncPreset * preset = m_manager->preset(index);
		Q_ASSERT_X(preset, "QyncMainWindow::showPreset(int)", "QyncManager provided null preset");
...
void QyncMainWindow::readPreferences( void ) {
	if(!m_manager) {
		qDebug() << "QyncMainWindow has no QyncManager";
		return;
	}

	const QyncGuiPreferences * prefs = dynamic_cast<const QyncGuiPreferences *>(m_manager->preferences());
	...
	if(prefs) {
		...
		if(prefs->showPresetsToolBar()) m_presetsToolbar->show();
		else m_presetsToolbar->hide();

		if(prefs->showSynchroniseToolBar()) m_synchroniseToolbar->show();
		else m_synchroniseToolbar->hide();
	}
}


void QyncMainWindow::syncIgnoreExistingToOnlyExisting( void ) {
...
void QyncMainWindow::syncOnlyExistingToIgnoreExisting( void ) {
...
void QyncMainWindow::disconnectManager( void ) {
	if(m_manager) {
...
void QyncMainWindow::connectManager( void ) {
	if(m_manager) {
...
QyncManager * QyncMainWindow::manager( void ) const {
	return m_manager;
}


bool QyncMainWindow::setManager( QyncManager * manager ) {
	disconnectManager();
	m_manager = manager;
	connectManager();
	refreshPresets();
...
void QyncMainWindow::refreshPresets( void ) {
	m_preset->clear();
	m_presetsMenu->clear();
	...
	if(m_manager) {
		...
		foreach(QyncPreset * preset, m_manager->presets()) {
...
void QyncMainWindow::chooseLogFile( void ) {
	QString newLog = QFileDialog::getSaveFileName(this, tr("Choose log file"), m_logFile->text());
	if(!newLog.isNull()) m_logFile->setText(newLog);
}


void QyncMainWindow::switchSourceAndDestination( void ) {
...
void QyncMainWindow::saveSettingsToCurrentPreset( void ) {
	...
	QyncPreset * oldPreset = m_manager->preset(i);
...
void QyncMainWindow::removeCurrentPreset( void ) {
...
void QyncMainWindow::newPresetFromSettings( void ) {
	bool ok;
...
void QyncMainWindow::exportPreset( void ) {
	...
	if(!fileName.isEmpty()) {
		...
		if(m_preset->count() > 0) {
			QyncPreset * myPreset = m_manager->preset(m_preset->currentIndex());
			if(myPreset) name = myPreset->name();
		}
...
void QyncMainWindow::fillPreset( QyncPreset * p ) const {
	if(!p) return;
...
void QyncMainWindow::simulate( void ) {
	fillPreset(m_tempPreset);
	QyncProcess * process = m_manager->simulate(m_tempPreset);

	if(process) {
		QyncProcessDialogue * w = new QyncProcessDialogue(process, this);
...
void QyncMainWindow::execute( void ) {
	fillPreset(m_tempPreset);
	QyncProcess * process = m_manager->execute(m_tempPreset);
...
void QyncMainWindow::showPreferences( void ) {
	if(!m_prefsWindow) {
		m_prefsWindow = new QyncPreferencesDialogue(m_manager, this);
...
void QyncMainWindow::about( void ) {
...
void QyncMainWindow::aboutRsync( void ) {
