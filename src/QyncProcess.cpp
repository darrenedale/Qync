/**
 * \file QyncProcess.cpp
 * \author Darren Hatherley
 * \date 13th December, 2013
 * \version 0.9.5
 *
 * \brief Implementation of the QyncProcess class.
 *
 * \dep
 * - QyncProcess.h
 * - QDebug
...
QyncProcess::QyncProcess( const QString & cmd, const QStringList & args, const QString & logFile )
:	QObject(),
	m_process(0),
	m_command(cmd),
	m_args(args),
...
QyncProcess::QyncProcess( const QString & cmd, const QyncPreset * preset )
:	QObject(),
	m_process(0),
	m_command(cmd),
	m_logFile(0) {
	...
	if(preset) {
		...
		m_args = QyncProcess::rsyncArguments(preset);
	}
}


QyncProcess::~QyncProcess( void ) {
...
QStringList QyncProcess::rsyncArguments( const QyncPreset * preset, const QStringList & forceOptions ) {
	...
	args << "--recursive" << "--progress" << "--verbose";

	/* basic settings */
	if(preset->preserveTime()) args << "--times";
	if(preset->preservePermissions()) args << "--perms";
	if(preset->preserveOwner()) args << "--owner";
	if(preset->preserveGroup()) args << "--group";

	if(preset->windowsCompatability()) args << "--modify-window=1";
	if(preset->honourDeletions()) args << "--delete";

	/* advanced settings */
	if(preset->alwaysCompareChecksums()) args << "--checksum";
	if(preset->preserveDevices()) args << "--devices";
	if(preset->keepPartialTransfers()) args << "--partial";
	if(preset->copySymlinksAsSymlinks()) args << "--links";
	if(preset->makeBackups()) args << "--backup";

	if(preset->useTransferCompression()) args << "--compress";
	if(preset->onlyUpdateExistingEntries()) args << "--existing";
	if(preset->dontUpdateExistingEntries()) args << "--ignore-existing";
	if(preset->dontMapUsersAndGroups()) args << "--numeric-ids";
	if(preset->copyHardlinksAsHardlinks()) args << "--hard-links";
	if(preset->showItemisedChanges()) args << "--itemize-changes";
...
QString QyncProcess::defaultExitCodeMessage( const QyncProcess::ExitCode & code ) {
	static QHash<QyncProcess::ExitCode, QString> s_messages;
...
QProcess * QyncProcess::process( void ) {
	return m_process;
}


QString QyncProcess::command( void ) const {
	return m_command;
}


QStringList QyncProcess::arguments( void ) const {
	return m_args;
}


QString QyncProcess::logFile( void ) const {
	return m_logFileName;
}


void QyncProcess::start( void ) {
	Q_ASSERT(m_process);
	...
	if(!m_logFileName.isEmpty()) {
		...
		if(!m_logFile->isOpen()) disposeLogFile();
	}
...
void QyncProcess::parseStderr( void ) {
	Q_ASSERT(m_process);
...
void QyncProcess::parseStdout( void ) {
	Q_ASSERT(m_process);
	//if(m_process->state() == QProcess::NotRunning) return;

	QString data = m_process->readAllStandardOutput();
	if(m_logFile && m_logFile->isOpen()) m_logFile->write(data.toUtf8());
...
/*			if(data.startsWith(' ')) {
		...
		if(lines.size() > 0) m_outputCache = lines.at(lines.size() - 1);
	}
}


void QyncProcess::disposeLogFile( void ) {
	if(m_logFile) {
		m_logFile->close();
		delete m_logFile;
		m_logFile = 0;
...
void QyncProcess::processFinished( void ) {
	Q_ASSERT(m_process);
	...
	switch(code) {
		...
		case UnableToWriteLogFile:
			emit(finished(QyncProcess::defaultExitCodeMessage(code)));
			break;

		case InterruptReceived:
			emit(interrupted(QyncProcess::defaultExitCodeMessage(code)));
			break;
		...
		case ConnectionTimeout:
			emit(failed(QyncProcess::defaultExitCodeMessage(code)));
			break;
	}
}
