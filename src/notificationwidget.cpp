...
namespace Qync {
	 ...
	 *
	 * This widget provides a pop-up widget that animates gracefully into view when it is
	 * shown and animates gracefully out again when dismissed. This provides the user with
	 * an experience that is not as jarring as popping up a dialogue window with a message
	 * immediately in front of the user. It does not get in the way of the existing UI or
	 * steal focus from it, and does not suddenly change the visuals in front of the user.
	 *
	 * It supports three types of notification: information, warning and error. Information
	 * messages are unobtrusive, just appearing like a normal label text; warning messages
	 * appear with a yellow background; error messages with a red background. The colours
	 * are relatively subtle, just enough to draw attention without jarring.
	 *
	 * The intended us for this widget is as a usually-hidden widget embedded in a layout,
	 * which is made visible when it has a message to show. It is suitable for relatively
	 * simple, short messages that the user can dismiss. They do not support any other user
	 * interaction, such as questions or alternative responses. The messages can be used
	 * with Qt's HTML-like markup for rich content.
	 *
	 * To use the widget call setMessage() then call show(). If the widget is already
	 * visible, it will be updated to show the provided content. If it is not visible, it
	 * will have the content set and will be gently scrolled into view.
	 */
	 ...
	 *
	 * If the widget is not currently visible, it will be gently scrolled into
	 * view. If it is already visible, or if the animation to scroll a previous
	 * message into view is in progress (i.e. show() has been called recently
	 * and its work has not yet finished), no action will be taken.
	 *
	 * This method is reimplemented from its base class but it is not virtual.
	 * This means that if the widget object is referenced by a base-class
	 ...
	 *
	 * If the widget is currently visible, it will be gently scrolled out of
	 * view. If it is already hidden, or if the animation to scroll it out of
	 * view is in progress (i.e. hide() has been called recently and its work
	 * has not yet finished), no action will be taken.
	 *
	 * This method is reimplemented from its base class but it is not virtual.
	 * This means that if the widget object is referenced by a base-class
