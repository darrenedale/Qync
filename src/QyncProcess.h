 ...
 * \version 0.9.5
 *
 * \brief Definition of the QyncProcess class.
 *
 * The QyncProcess class takes the command and arguments from the manager
 * rather than just being given the preset. This is because the preset is
 * owned by the manager and could be removed while the QyncProcess object is
 * still alive. In this scenario, the QyncProcess object would retain a
 * pointer to an object that no longer exists, and therefore any attempt by
 * the QyncProcess object to access the preset would result in a segfault.
 * Therefore, the code to build the rsync command must remain in the QyncManager
 * class.
 *
 * \dep
 * - QObject
...
#ifndef QYNCPROCESS_H
#define QYNCPROCESS_H
...
:	public QObject {
	...
	public:
		...
		/**
		 * \brief Create a new QyncProcess.
		 *
		 ...
		 */
		QyncProcess( const QString & cmd, const QStringList & args, const QString & logFile = QString() );
		 ...
		 */
		QyncProcess( const QString & cmd, const QyncPreset * preset );

		/**
		 * \brief Destroy the QyncProcess.
		 */
		virtual ~QyncProcess( void );
		 ...
		 * returned list.
		 *
		 * Given a QyncPreset object, this method will produce the set of
		 ...
		 */
		static QStringList rsyncArguments( const QyncPreset * preset, const QStringList & forceOptions = QStringList() );
		 ...
		 */
		static QString defaultExitCodeMessage( const QyncProcess::ExitCode & code );
	...
	signals:
		 ...
		 */
		void finished( QyncProcess::ExitCode code );
